...
class sidebar_generator {
  ...
  function admin_menu(){
    add_theme_page(__( 'Sidebars', 'editit' ), __( 'Sidebars', 'editit' ), 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));
  }
  ...
  function admin_page(){
    ...
    <script>
      function remove_sidebar_link(name,num){
        answer = confirm("<?php _e( 'Are you sure you want to remove', 'editit' )  ?> " + name + "<?php _e( '?\nThis will remove any widgets you have assigned to this sidebar.', 'editit' )  ?>");
        if(answer){
          //alert('AJAX REMOVE');
      ...
      function add_sidebar_link(){
        var sidebar_name = prompt("<?php _e( 'Sidebar Name', 'editit' )  ?>:","");
        //alert(sidebar_name);
        add_sidebar(sidebar_name);
      }
    </script>
    <div class="wrap">
      <h2><?php _e( 'Sidebar Generator', 'editit' )  ?></h2>
      <p>
        <?php _e( 'The sidebar name is for your use only. It will not be visible to any of your visitors. A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'editit' )  ?>
      </p>
      <br />
      <div class="add_sidebar">
        <a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar">+ <?php _e( 'Add Sidebar', 'editit' )  ?></a>
      </div>
      <br />
      <table class="widefat page" id="sbg_table" style="width:600px;">
        <tr>
          <th><?php _e( 'Name', 'editit' )  ?></th>
          <th><?php _e( 'CSS class', 'editit' )  ?></th>
          <th><?php _e( 'Remove', 'editit' )  ?></th>
        </tr>
        <?php
        $sidebars = sidebar_generator::get_sidebars();
        ...
        <tr class="<?php echo $alt?>">
          ...
          <td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="<?php _e( 'Remove this sidebar', 'editit' )  ?>"><?php _e( 'remove', 'editit' )  ?></a></td>
        </tr>
        <?php
            $cnt++;
          }
          ...
          ?>
          <tr>
            <td colspan="3"><?php _e( 'No Sidebars defined', 'editit' )  ?></td>
          </tr>
          <?php
        }
        ?>
  ...
  */
  ...
  <div id='sbg-sortables' class='meta-box-sortables'>
    <div id="sbg_box" class="postbox " >
      <div class="handlediv" title="Click to toggle"><br /></div><h3 class='hndle'><span><?php _e( 'Sidebars', 'editit' )  ?></span></h3>
      <div class="inside">
        <div class="sbg_container">
          <input name="sbg_edit" type="hidden" value="sbg_edit" />
          <p><?php _e( 'Select the sidebar you wish to display on this page. If you don&apos;t want to display a Sidebar, select the "No Sidebar" at Layout Setting.<br /><strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars. ', 'editit' )  ?>
          </p>
          <ul>
          <?php 
